...
export interface GalleryImage {
  ...
  url: string;
  alt: string;
  brand: 'general' | 'desembre' | 'dermagarden';
  caption?: string;
  isActive: boolean;
}
...
export type SiteSettingsRow = {
  ...
  home_product_gallery_images?: any; // raw JSONB column
};
...
export function mapSiteSettings(row: SiteSettingsRow): SiteSettings {
  return {
    ...
    homeProductGalleryImages: row.home_product_gallery_images ? JSON.parse(row.home_product_gallery_images as any) : [],
  };
}
...
export async function updateSiteSettings(payload: Partial<SiteSettings>): Promise<{ ok: boolean; error?: string }> {
  ...
  if (payload.homeProductGalleryImages !== undefined) updateData.home_product_gallery_images = JSON.stringify(payload.homeProductGalleryImages);
